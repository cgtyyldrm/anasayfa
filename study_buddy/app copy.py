...
def get_turkey_time():
    try:
        ...
        return date.today()
# --- SESSION MANAGER (AKILLI HAFIZA) ---
def save_session_state():
    """Sayacı çereze kaydeder (Sayfa yenilenirse hatırlamak için)"""
    state_data = {
        "isActive": True,
        "isRunning": st.session_state.timer_running,
        "startTime": st.session_state.timer_start_time, # Başlangıç zamanı
        "accumulated": st.session_state.timer_accumulated, # Biriken süre
        "taskInfo": st.session_state.current_task_info, # Hangi ders?
        "readingDuration": st.session_state.get("reading_duration", 15),
        "stats": {
            "dogru": st.session_state.temp_dogru,
            "yanlis": st.session_state.temp_yanlis,
            "bos": st.session_state.temp_bos
        },
        "lastSaved": time.time()
    }
    # Veriyi 1 gün boyunca tarayıcıda sakla
    cookie_manager.set("study_session_v2", state_data, expires_at=datetime.now() + timedelta(days=1))

def load_session_state():
    if st.session_state.get("timer_active", False): return

    saved = cookie_manager.get("study_session_v2")
    if saved and saved.get("isActive"):
        st.session_state.timer_active = True
        st.session_state.current_task_info = saved.get("taskInfo", {})
        st.session_state.reading_duration = saved.get("readingDuration", 15)
        
        stats = saved.get("stats", {})
        st.session_state.temp_dogru = stats.get("dogru", 0)
        st.session_state.temp_yanlis = stats.get("yanlis", 0)
        st.session_state.temp_bos = stats.get("bos", 0)
        
        st.session_state.timer_running = saved.get("isRunning", False)
        st.session_state.timer_accumulated = saved.get("accumulated", 0)

        # Eğer sayaç çalışıyorken sayfa kapandıysa, aradaki geçen zamanı kaybetmemek için:
        if st.session_state.timer_running:
            # Başlangıç zamanını 'şimdi' olarak set edip, birikmiş süreyi koruyoruz
            st.session_state.timer_start_time = time.time()
            
        st.toast("🔄 Oturum kurtarıldı!", icon="✅")

def clear_session_state():
    """İş bitince hafızayı temizler"""
    cookie_manager.delete("study_session_v2")
...
def main_app():
    ...
    if st.session_state.timer_active:
        ...
        with c_focus_2:
            ...
            # --- ZAMAN HESAPLAMA ---
            # Eğer çalışıyorsa: Birikmiş Süre + (Şu an - Başlangıç)
            # Eğer durduysa: Sadece Birikmiş Süre
            if st.session_state.timer_running:
                elapsed_seconds = st.session_state.timer_accumulated + (current_time - st.session_state.timer_start_time)
            else:
                elapsed_seconds = st.session_state.timer_accumulated

# --- KİTAP OKUMA MODU ---
            if is_reading_mode:
                if "reading_duration" not in st.session_state:
                    st.session_state.reading_duration = 15 
                
                # Sayaç başlamadıysa süre seçtir
                if not st.session_state.timer_running and st.session_state.timer_accumulated == 0:
                     st.info("Okuma süresini seç:")
                     dur = st.slider("Dakika", 1, 60, 15, step=5)
                     st.session_state.reading_duration = dur
                
                target_seconds = st.session_state.get("reading_duration", 15) * 60
                remaining = max(0, target_seconds - elapsed_seconds)
                
# --- SAYAÇ BİTİŞ: TİTREŞİM VE GÖRSEL UYARI ---
                if remaining == 0 and st.session_state.timer_running:
                     st.session_state.timer_running = False
                     st.session_state.timer_accumulated = elapsed_seconds
                     clear_session_state() # İş bitti, hafızayı temizle
            ...
            with col_btn1:
                if st.session_state.timer_running:
                    if st.button("⏸️ Mola Ver", use_container_width=True):
                        st.session_state.timer_accumulated = elapsed_seconds
                        st.session_state.timer_running = False
                        save_session_state()  # <--- BURAYA EKLE (Mola verince kaydet)
                        st.rerun()
                else:
                    # Bitti mi kontrolü
                    is_finished = (is_reading_mode and remaining == 0 and st.session_state.timer_accumulated > 0)
                    if not is_finished:
                        btn_text = "▶️ Başla" if st.session_state.timer_accumulated == 0 else "▶️ Devam Et"
                        if st.button(btn_text, type="primary", use_container_width=True):
                            st.session_state.timer_start_time = time.time()
                            st.session_state.timer_running = True
                            save_session_state()  # <--- BURAYA EKLE (Başlayınca kaydet)
                            st.rerun()
            ...
            with col_btn2:
                ...
                if st.button(finish_label, type="primary", use_container_width=True):
                    ...
                    # TEMİZLİK BURADA BAŞLIYOR
                    clear_session_state() # <--- Çerezdeki oturumu siler
                    ...
                    if "reading_duration" in st.session_state: 
                        del st.session_state.reading_duration
            ...
            if not is_reading_mode and st.button("💾 Kaydet ve Çık (Bitmedi)", use_container_width=True):
                ...
                clear_session_state() # <--- BURAYA EKLE (Bitince kaydı temizle)
                st.session_state.timer_active = False
                st.session_state.timer_running = False
                st.session_state.timer_accumulated = 0
                st.session_state.temp_dogru = 0
